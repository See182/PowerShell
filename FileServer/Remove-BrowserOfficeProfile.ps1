<#
    .SYNOPSIS
    Remove Microsoft Internet Explorer/Edge, Google Chrome, Mozilla Firefox and Office 365 Settings and Profiles.

    .DESCRIPTION
    Synchronizes all Settings and Profiles from local drive to a network drive. Apps included are Microsoft Internet Explorer/Edge, Google Chrome, Mozilla Firefox and Office 365.
...
if (Test-Path $OutlookProfile) {
    ...
    Copy-Item "$($OutlookProfile)\Office\*.acl" -Destination "$($ServerPath)\$($OfficeFolder)\$($OutlookAutoCorrectionLists)\" -Recurse
    ...
    Copy-Item "$($OutlookPath)\16.0\outlook.exe_Rules.xml" -Destination "$($ServerPath)\$($OfficeFolder)\$($OutlookRulesXML)\" -Recurse
    ...
    Copy-Item "$($OutlookPath)\*.officeUI" -Destination "$($ServerPath)\$($OfficeFolder)\$($OutlookOfficeUI)\" -Recurse
